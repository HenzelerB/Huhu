...
Down_id_mapping <- read.delim("idmapping_2024_11_13.tsv", header = TRUE, sep = "\t", stringsAsFactors = FALSE)
m.df.down <- m.df %>%
  filter(Regulation=="Down")%>%
  left_join(Down_id_mapping, by = c("Uniprot_ID" = "Entry"))
write.csv(m.df.down,"FAnn_down.csv")

Up_id_mapping <- read.delim("idmapping_2024_11_13.tsv", header = TRUE, sep = "\t", stringsAsFactors = FALSE)
m.df.ups <- m.df %>%
